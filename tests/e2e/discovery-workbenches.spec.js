...
const datasets = [{
  ...
  tags: ['task_categories:text-retrieval', 'size_categories:10K<n<100K', 'language:zh', 'license:apache-2.0'],
  downloads: 4200,
  likes: 120,
  trendingScore: 80,
...
test('Datasets sends exact, independently combinable source filters', async ({ page }) => {
  ...
  await page.getByLabel('Dataset size').selectOption('10k-100k');
  ...
  await page.getByLabel('Benchmark').selectOption('official');
  ...
  expect(request.searchParams.getAll('filter')).toEqual(expect.arrayContaining([
    ...
    'language:zh', 'license:apache-2.0', 'benchmark:official',
  ]));
